...
def NullifyQs(NN,env):
    ...
    if np.linalg.matrix_rank(statesLeftHand) < stateCount:
        ...
        statesLeftHand+=np.mean(statesLeftHand)*0.1*np.random.rand(statesLeftHand.shape[0],statesLeftHand.shape[1])
...
def CreateFeaturizer(env):
    ...
    observation_examples = np.squeeze(np.array([env.step(np.random.randint(env.nA))[0] for x in range(10000)]))
...
##        actuators = [[0] ,LinAct]
        ...
        NeuralNet = network(epochs,tolerance,actuators,layer_parameters,layer_types,
                          learningRate,beta1,beta2,epsilon,lam,learningDecay = learningDecay,costFunctionType='L2')
    ...
    if saveFile != None:
        # Unpickle -  Data = [wv,bv]
        NN.wv,NN.bv = Unpickler(saveFile + ".dat"); print('\nVariables loaded from ' + saveFile + '.dat')
