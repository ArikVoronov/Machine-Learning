...
def EvoFitnessFunction(clf,env,agent):
    def AgentFitness(gen):
        halfGen=int(len(gen)/2)
        clf.wv = gen[0:halfGen]
        clf.bv = gen[halfGen:]
        state = env.reset()
        totalReward = 0
        done = False
...
class GAOptimizer():
    def __init__(self,specimenCount =2000,survivorCount=20,tol = 1E-5,maxIterations = 300,mutationRate = 0.1,generationMethod = "Random Splice",smoothing = 5):
        ...
        self.childrenCount = int(self.specimenCount/self.survivorCount) # calculated number of children for each survivor
        self.wildChildrenCount = np.max([int(self.childrenCount/10),1]) # Wild mutation rate - set to 10% but not less than 1
...
if __name__ == "__main__":
    ...
    def InitNN(x,y):
        ...
        NeuralNet = network(epochs,tolerance,actuators,layer_parameters,layer_types,alpha,beta1,beta2,epsilon,gamma,lam,lossFunctionType)
    ...
    gao1 = GAOptimizer(specimens =2000,survivorCount=20,tol = 1E-5,maxIterations = 450,mutationRate = 0.02,generationMethod = "Random Splice")
    ...
    weights = np.random.rand(features)
    biases = np.random.rand(1)
    gao2 = GAOptimizer(specimens =2000,survivorCount=20,tol = 1E-5,maxIterations = 450,mutationRate = 0.02,generationMethod = "Pure Mutation")
    ...
    NNGA.initialize()
    gao3 = GAOptimizer(specimens =500,survivorCount=10,tol = 1E-5,maxIterations = 5000,mutationRate = 0.05,generationMethod = "Random Splice")
    ...
    plt.close('all')
    plt.figure(2)
    plt.plot(fv);    plt.plot(fv2);    plt.plot(fv3)
    plt.yscale('log')
    plt.grid()
    plt.ylabel('Fitness');plt.xlabel('Iteration')
