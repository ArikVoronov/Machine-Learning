...
class Obstacle():
    ...
    def Collide(self,obj):
        if np.abs(self.pos[0] - obj.pos[0] ) <= self.tol+(self.size+obj.size)/2:
            if np.abs(self.pos[1] - obj.pos[1]) <= self.tol:
             self.collide = True
    ...
    def GetReward(self,action):
        ...
        if self.done:
            ...
            for i,o in enumerate(self.obstacleList):
                if np.abs(self.player.pos[0]-o.pos[0])<self.obsVelocity:
                    reward += 1
##                    print('hop')
